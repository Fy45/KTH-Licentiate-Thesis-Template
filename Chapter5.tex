...
\footnotetext[1]{American call option is defined as the right of contract holder to  purchase shares at the specified price and quantity within the validity period stipulated in the agreement. In our cases, the exchange rate itself is unsteady  during time, so within the contract time, the initiator could redeem the contract at the time when it is in best interests. Thus, causing another party to lose money.}
...
\begin{itemize}
	...
	\item The conclusion driven from~\nameref{chap:4} is based on two different test applications that does not apply in the same use scenario, although the result is valid because we evaluate the working mechanism. We could be benefit from getting more information through one common test environment in the future.
